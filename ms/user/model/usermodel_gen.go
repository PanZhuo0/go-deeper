...
var (
	...
	cacheBookUserUserIdPrefix   = "cache:book:user:userId:"
	cacheBookUserUsernamePrefix = "cache:book:user:username:"
)
...
func (m *defaultUserModel) Delete(ctx context.Context, id int64) error {
	...
	bookUserUserIdKey := fmt.Sprintf("%s%v", cacheBookUserUserIdPrefix, data.UserId)
	bookUserUsernameKey := fmt.Sprintf("%s%v", cacheBookUserUsernamePrefix, data.Username)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, bookUserIdKey, bookUserUserIdKey, bookUserUsernameKey)
	return err
}
...
func (m *defaultUserModel) FindOneByUserId(ctx context.Context, userId int64) (*User, error) {
	bookUserUserIdKey := fmt.Sprintf("%s%v", cacheBookUserUserIdPrefix, userId)
	var resp User
	err := m.QueryRowIndexCtx(ctx, &resp, bookUserUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultUserModel) FindOneByUsername(ctx context.Context, username string) (*User, error) {
	bookUserUsernameKey := fmt.Sprintf("%s%v", cacheBookUserUsernamePrefix, username)
	var resp User
	err := m.QueryRowIndexCtx(ctx, &resp, bookUserUsernameKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
